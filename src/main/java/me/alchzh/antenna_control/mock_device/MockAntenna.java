...
public class MockAntenna {
    ...
    public MockAntenna(Forwarder forwarder, int baseAz, int baseEl, int minAz, int minEl, int maxAz, int maxEl, int speed) {
    ...
    public void commandReceived(AntennaCommand command) {
    ...
    private void send(AntennaEvent.Type eventType, byte... data) {
        forwarder.sendEvent(new AntennaEvent(eventType, getTimeElapsed(), data));
    }
    ...
    @FunctionalInterface
    interface Forwarder {
        void sendEvent(AntennaEvent event);
    }
}
